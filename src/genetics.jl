...
function createtraits()
    ...
    dispmoffset = setting("dispmean") - setting("mindispmean")
    dispsoffset = setting("dispshape") - setting("mindispshape")
    ...
    for idx in eachindex(traitnames)
        if occursin("dispshape", traitnames[idx])
            traits[idx] = Trait(idx, setting("mindispshape") + rand() * dispsoffset)
        elseif occursin("dispmean", traitnames[idx])
            traits[idx] = Trait(idx, setting("mindispmean") + rand() * dispmoffset)
