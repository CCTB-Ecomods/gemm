...
function writesettings()
    ...
    open(joinpath(setting("dest"), settingspath), "w") do f
        ...
        println(f, "# "*read(`git rev-parse HEAD`, String)*"\n")
...
function writefasta(world::Array{Patch,1}, timestep::Int)
    ...
    if setting("dumpindforfasta")
        filename_ind = "inds_s"* string(setting("seed"))
        filename_ind = joinpath(setting("dest"), filename_ind)
        filename_ind = filename_ind * ".tsv"
        simlog("Writing individual data \""*string(filename_ind)*"\"")
...
macro simlog(msg::Union{String,Expr}, category::Char='i', args...)
    return :((($category != 'd') || setting("debug")) && simlog(:($$(esc(msg))), $category, $(args...)))
end
